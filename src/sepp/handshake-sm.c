        ...
        CASE(OGS_SBI_SERVICE_NAME_N32C_HANDSHAKE)
            ...
            DEFAULT
                ogs_error("[%s] Invalid resource name [%s]",
                        sepp_node->fqdn, message->h.resource.component[0]);
            END
            break;

        DEFAULT
            ogs_error("[%s] Invalid API name [%s]",
                    sepp_node->fqdn, message->h.service.name);
        END
        break;
                ...
                CASE(OGS_SBI_HTTP_METHOD_POST)
                    ...
                    if (message->res_status == OGS_SBI_HTTP_STATUS_OK) {
                        ...
                        ogs_error("[%s] HTTP Response Status Code [%d]",
                                sepp_node->fqdn, message->res_status);
                    }
                    break;
...
void sepp_handshake_state_established(ogs_fsm_t *s, sepp_event_t *e)
{
    ...
    switch (e->h.id) {
    case OGS_FSM_ENTRY_SIG:
        ogs_info("[%s] SEPP established", sepp_node->fqdn);
        break;

    case OGS_FSM_EXIT_SIG:
        ogs_info("[%s] SEPP terminated", sepp_node->fqdn);
        break;
    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_N32C_HANDSHAKE)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_EXCHANGE_CAPABILITY)
                ...
                CASE(OGS_SBI_HTTP_METHOD_POST)
                    if (message->res_status == OGS_SBI_HTTP_STATUS_OK) {
                        ogs_warn("[%s] SEPP was terminated", sepp_node->fqdn);
                    } else {
                        ogs_error("[%s] HTTP Response Status Code [%d]",
                                sepp_node->fqdn, message->res_status);
                    }
...
void sepp_handshake_state_exception(ogs_fsm_t *s, sepp_event_t *e)
{
    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_N32C_HANDSHAKE)
            ...
            SWITCH(message->h.resource.component[0])
            CASE(OGS_SBI_RESOURCE_NAME_EXCHANGE_CAPABILITY)
                ogs_error("[%s] SEPP exception state", sepp_node->fqdn);
                break;
            DEFAULT
