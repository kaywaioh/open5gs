        ...
        if (!strcmp(root_key, "db_uri")) {
            ...
            while (ogs_yaml_iter_next(&parameter_iter)) {
                ...
                if (!strcmp(parameter_key, "no_hss")) {
                    ...
                    ogs_app()->use_mongodb_change_stream =
                        ogs_yaml_iter_bool(&parameter_iter);
#else
                    ogs_app()->use_mongodb_change_stream = false;
#endif
                } else
            ...
            while (ogs_yaml_iter_next(&pool_iter)) {
                ...
                if (!strcmp(pool_key, "128")) {
                    ...
                    if (v)
                        ogs_app()->pool.defconfig.cluster_128_pool = atoi(v);
                } else if (!strcmp(pool_key, "256")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_256_pool = atoi(v);
                } else if (!strcmp(pool_key, "512")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_512_pool = atoi(v);
                } else if (!strcmp(pool_key, "1024")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_1024_pool = atoi(v);
                } else if (!strcmp(pool_key, "2048")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_2048_pool = atoi(v);
                } else if (!strcmp(pool_key, "8192")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_8192_pool = atoi(v);
                } else if (!strcmp(pool_key, "32768")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_32768_pool = atoi(v);
                } else if (!strcmp(pool_key, "big")) {
                    const char *v = ogs_yaml_iter_value(&pool_iter);
                    if (v)
                        ogs_app()->pool.defconfig.cluster_big_pool = atoi(v);
                } else
